  ...
  if (notFound) {
    return (
      <div className="landing-page static-page mesh-gradient-bg">
        ...
        <LandingFooter schoolCount={allSchools.length || 49} onOpenLogin={() => router.push('/?login=1')} />
      </div>
    );
  }
                ...
                <RevealOnScroll key={i} direction="right" delay={i * 0.05} duration={0.6}>
                  <SpotlightCard style={{ borderRadius: 20 }}>
                    <div className="staff-inner">
                      <div className="staff-avatar" style={i === 1 ? { background: `linear-gradient(135deg, ${theme.accent}, ${theme.primary})` } : {}}><i className={`fa-solid ${s.icon}`} aria-hidden="true" /></div>
                      <div>
                ...
                <RevealOnScroll key={i} delay={i * 0.1} duration={0.6}>
                  <div className="timeline-item">
                    ...
                    <SpotlightCard className="timeline-card-inner" style={{ borderRadius: 16 }}>
                      <h4 className="timeline-card-title"><i className={`fa-solid ${m.icon}`} aria-hidden="true" /> {m.title}</h4>
                ...
                {facilities.length > 0 ? facilities.map((fac, i) => (
                  <RevealOnScroll key={fac.id} delay={i * 0.04} duration={0.5}>
                    <div className="facility-bento-card" title={fac.description || fac.name}>
                      <div className="facility-bento-icon"><i className={`fa-solid ${fac.icon}`} aria-hidden="true" /></div>
                  ...
                  return (
                    <RevealOnScroll key={ach.id} delay={i * 0.05} duration={0.6}>
                      <SpotlightCard style={{ borderRadius: 16 }}><div className="feat-inner">
                        <div className="feat-icon" style={{ background: `${ac}10`, color: ac }}><i className={`fa-solid ${ach.icon}`} aria-hidden="true" /></div>
                ...
                <div className="carousel-track" style={{ transform: `translateX(-${carouselIdx * 100}%)` }}>
                  {testimonials.map((t, i) => (
                    <div key={i} className="carousel-slide">
                      <div className="carousel-avatar"><i className={`fa-solid ${t.icon}`} aria-hidden="true" /></div>
                  ...
                  return (
                    <RevealOnScroll key={link.key} delay={i * 0.05} duration={0.5}>
                      <a href={href} target="_blank" rel="noopener noreferrer" className="social-card">
                        <div className="social-icon" style={{ background: link.color }}><i className={`${link.brand ? 'fa-brands' : 'fa-solid'} ${link.icon}`} aria-hidden="true" /></div>
                ...
                <div className="qr-section">
                  ...
                  <div className="qr-info">
                    ...
                    <MagneticButton className="btn-outline" onClick={handleShare} style={{ padding: '10px 20px', fontSize: 13 }}>
                      <i className={`fa-solid ${copied ? 'fa-check' : 'fa-copy'}`} aria-hidden="true" /> {copied ? 'Link Tersalin!' : 'Salin Link Profil'}
                    </MagneticButton>
                  </div>
                </div>
              </SpotlightCard>
