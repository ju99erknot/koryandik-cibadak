...
export default function SekolahDirectoryPage() {
  ...
  return (
    <div className="landing-page static-page mesh-gradient-bg">
        ...
        {!loaded ? (
          <div className={viewMode === 'grid' ? 'skl-grid' : viewMode === 'list' ? 'skl-list' : 'skl-compact'}>
            {Array.from({ length: 6 }).map((_, i) => (
              <div key={i} className="content-skeleton" style={{ height: viewMode === 'grid' ? 340 : viewMode === 'list' ? 72 : 64 }} />
            ))}
          </div>
        ) : filtered.length === 0 ? (
            ...
            {filtered.map((school, i) => {
              ...
              return (
                <RevealOnScroll key={school.npsn} delay={(i % 4) * 0.05} duration={0.45}>
                  <div className="skl-card" style={{ ...cardVars, ['--reveal-delay' as string]: `${(i % 6) * 60}ms`, height: '100%' }}>
                    {/* Card Header */}
                    <div className="skl-card-header">
                      ...
                      <div className="skl-card-header-row">
                        <div className="skl-card-logo" style={{ background: school.logoUrl ? 'var(--card-glass)' : `linear-gradient(135deg,${theme.primary},${theme.accent})` }}>
                          {school.logoUrl
                            ? <img src={school.logoUrl} alt={school.name} style={{ width: '100%', height: '100%', objectFit: 'contain', borderRadius: 12 }} />
                            : <i className="fa-solid fa-graduation-cap"  aria-hidden="true"/>}
                        </div>
                        ...
                        {(() => {
                          ...
                          return (
                            <span className="skl-badge" style={{
                              ...
                              <i className={status === 'Negeri' ? 'fa-solid fa-landmark' : 'fa-solid fa-building'} style={{ fontSize: '9px' }} aria-hidden="true" />
                              {status}
                            </span>
                          );
                        })()}
              ...
              return (
                ...
                >
                  <div className="skl-list-logo" style={{ background: school.logoUrl ? 'var(--card-glass)' : `linear-gradient(135deg,${theme.primary},${theme.accent})` }}>
                    {school.logoUrl
                      ? <img src={school.logoUrl} alt={school.name} style={{ width: '100%', height: '100%', objectFit: 'contain', borderRadius: 10 }} />
                      : <i className="fa-solid fa-graduation-cap"  aria-hidden="true"/>}
                  </div>
                  <div style={{ flex: 1, minWidth: 0 }}>
              ...
              return (
                ...
                >
                  <div className="skl-compact-logo" style={{ background: school.logoUrl ? 'var(--card-glass)' : `linear-gradient(135deg,${theme.primary},${theme.accent})` }}>
                    {school.logoUrl
                      ? <img src={school.logoUrl} alt={school.name} style={{ width: '100%', height: '100%', objectFit: 'contain', borderRadius: 8 }} />
                      : <i className="fa-solid fa-graduation-cap"  aria-hidden="true"/>}
                  </div>
                  <div style={{ flex: 1, minWidth: 0 }}>
      ...
      <LandingFooter schoolCount={schools.length || 49} onOpenLogin={() => router.push('/?login=1')} />
      <BackToTop />
    </div>
  );
}
