                ...
                <form onSubmit={handleSubmit}>
                  ...
                  <div className="form-group">
                    ...
                    <div role="radiogroup" aria-labelledby="announcement-priority-label" style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                      {[
                        { value: 'low' as const, label: 'ℹ️ Info', color: 'var(--text-muted)', bg: 'rgba(148, 163, 184, 0.1)', border: 'rgba(148, 163, 184, 0.2)' },
                        { value: 'normal' as const, label: '📋 Normal', color: 'var(--primary)', bg: 'rgba(59, 130, 246, 0.1)', border: 'rgba(59, 130, 246, 0.25)' },
                        { value: 'high' as const, label: '🚨 Mendesak', color: 'var(--danger)', bg: 'rgba(239, 68, 68, 0.1)', border: 'rgba(239, 68, 68, 0.3)' }
                      ].map(opt => (
                        <button
                          key={opt.value}
                          type="button"
                  ...
                  <button type="submit" className="btn btn-block" style={{
                    background: priority === 'high' ? 'linear-gradient(135deg, var(--danger), var(--danger))' : '',
                ...
                {announcements.map(ann => (
                  <div
                    ...
                    style={{
                      ...
                      borderLeft: `4px solid ${ann.priority === 'high' ? 'var(--danger)' : ann.priority === 'normal' ? 'var(--primary)' : 'var(--text-muted)'}`,
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '10px' }}>
                      <div>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                          {ann.priority === 'high' && (
                            <i className="fa-solid fa-tower-broadcast" style={{ color: 'var(--danger)', fontSize: '14px', animation: 'pulse 1s infinite' }} aria-hidden="true"></i>
                          )}
