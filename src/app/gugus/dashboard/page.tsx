...
export default function GugusDashboard() {
  ...
  const donutData = useMemo(() => [
    { name: 'Disetujui', value: stats.approved, color: 'var(--success)' },
    { name: 'Menunggu', value: stats.pending, color: 'var(--warning)' },
    { name: 'Revisi', value: stats.revision, color: 'var(--purple)' },
  ].filter(d => d.value > 0), [stats]);

  if (!gugusUser) return <LoadingScreen />;
  ...
  return (
    ...
    >
        <div className="content-area">
          ...
          <div className="stats-grid">
            <div className="stat-card">
              ...
              <div className="stat-info">
                <span className="stat-value">{stats.total.toLocaleString('id-ID')}</span>
            ...
            <div className="stat-card">
              ...
              <div className="stat-info">
                <span className="stat-value">{stats.approved.toLocaleString('id-ID')}</span>
            ...
            <div className="stat-card">
              ...
              <div className="stat-info">
                <span className="stat-value">{stats.pending.toLocaleString('id-ID')}</span>
            ...
            <div className="stat-card">
              ...
              <div className="stat-info">
                <span className="stat-value">{stats.revision.toLocaleString('id-ID')}</span>
                <span className="stat-label">Revisi</span>
              </div>
            </div>
          </div>

          {/* ===== ANALYTICS + DONUT ROW (Equal Height Bento Pair) ===== */}
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: '24px', marginBottom: '24px', alignItems: 'stretch' }}>
          ...
          {/* ===== ANTREAN VERIFIKASI ===== */}
          <div id="antrean-verifikasi" style={{ scrollMarginTop: 90, marginBottom: '24px' }}>
            <VerificationQueue
              submissions={allSubmissions}
              schools={schools}
              categories={categories}
              reviewerName={gugusUser?.name || 'Koordinator Gugus'}
              restrictToNpsns={schoolNpsns}
                                                ...
                                                <div style={{ display: 'flex', gap: '4px', flexWrap: 'wrap' }}>
                                                  ...
                                                  {sub.status !== 'approved' && (
                                                    <button type="button" className="btn btn-xs" onClick={() => handleApprove(sub.id)} title="Setujui"
                                                      style={{ background: 'var(--success)', borderColor: 'var(--success)', color: '#ffffff' }}>
