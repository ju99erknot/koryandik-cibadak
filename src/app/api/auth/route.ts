...
async function verifyCredentials(
  ...
  if (role === 'gugus') {
    // Check Supabase dynamic gugus data first
    if (process.env.NEXT_PUBLIC_SUPABASE_URL && process.env.NEXT_PUBLIC_SUPABASE_URL !== 'https://placeholder.supabase.co') {
      try {
        const { data } = await supabase
          .from('gugus')
          .select('id, sekolah_inti, passcode')
          .eq('id', identifier)
          .maybeSingle();

        if (data) {
          const validCodes: string[] = [];
          if (data.sekolah_inti) validCodes.push(data.sekolah_inti);
          if (data.passcode) validCodes.push(data.passcode);

          if (validCodes.some((code) => safeEqual(passcode, code))) {
            return { role: 'gugus', sub: identifier };
          }
        }
      } catch (err) {
        console.warn('[auth] Supabase gugus check failed, falling back to static secrets', err);
      }
    }

    // Static fallback
    const gugus = gugusSecrets.find((g) => g.id === identifier);
    if (!gugus) return null;
    const passcodes = Array.isArray(gugus.passcode) ? gugus.passcode : [gugus.passcode];
    if (!passcodes.some((code) => safeEqual(passcode, code))) return null;
...
export async function POST(req: Request) {
  try {
    ...
    const identity = await verifyCredentials(role, identifier, passcode);
